...
export default function Memorial({ actions = [] }: Props) {
  ...
  const { currentMonth, terroristActions, initDate, endDate } = useMemo(() => {
    if (!isMounted) {
      return { currentMonth: "", terroristActions: [], initDate: null, endDate: null };
    }
    ...
    const startYear = actionsDates.length > 0 ? actionsDates[0].getFullYear() : null;
    const endYear = actionsDates.length > 0 ? actionsDates[actionsDates.length - 1].getFullYear() : null;
  ...
  if (terroristActions.length === 0) {
      return (
        <div className="mt-8 border border-border/50 p-12 rounded-sm text-center lg:text-left animate-in fade-in duration-700">
            ...
            <h3 className={`${newsreader.className} text-2xl font-bold uppercase mb-4`}>Sin efemérides para {currentMonth}</h3>
            <p className="text-muted-foreground italic mb-8 max-w-md">
                No hay registros documentados para este mes en particular. Te invitamos a consultar el archivo cronológico completo.
            </p>
            <Link
                href="/efemerides"
  ...
  return (
    <div className="mt-8 animate-in fade-in duration-700">
      <header className="mb-6 border-b-2 border-primary/20 pb-4">
        <h2 className={`${newsreader.className} text-3xl md:text-4xl font-black uppercase tracking-tighter text-center lg:text-left`}>
          Memorial del Mes de <span className="text-primary italic">{currentMonth}</span>
        </h2>
        <p className={`${newsreaderItalic.className} text-lg text-muted-foreground mt-2 text-center lg:text-left`}>
      ...
      <div className="flex flex-col items-center lg:items-start gap-8">
        <figure className="relative group w-full">
          ...
          <figcaption className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground mt-4 border-l-2 border-primary pl-3">
