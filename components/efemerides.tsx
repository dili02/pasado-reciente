...
function TimelineItem({ action }: { action: TerroristActionDefinition }) {
  ...
  const monthName = new Intl.DateTimeFormat("es-UY", { month: "short" })
    .format(dateObj)
    .toUpperCase()
    .replace(".", "");
    ...
    <article className="group relative border-b border-border/50 pb-8 pt-8 first:pt-0 last:border-0 transition-all hover:bg-muted/30 px-4 -mx-4 rounded-sm">
      ...
      >
        ...
        <div className="flex-shrink-0 flex md:flex-col items-center justify-center w-full md:w-20 py-2 border-2 border-primary/20 rounded-sm bg-muted/50 group-hover:bg-primary group-hover:border-primary transition-all duration-300">
          <span className="text-lg md:text-2xl font-black leading-none group-hover:text-white transition-colors">
            {day}
          </span>
          <span className="text-lg lg:text-[12px] font-black uppercase tracking-widest mt-1 opacity-60 group-hover:text-white group-hover:opacity-100 transition-all">
            {monthName}
          </span>
          <div className="h-px w-8 bg-primary/20 group-hover:bg-white/40 my-1 hidden md:block" />
          <span className="text-lg lg:text-[14px] font-black group-hover:text-white transition-colors">
            {year}
          </span>
        </div>
        ...
        <div className="flex-1 space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-6 pt-2">
            ...
            <div className="flex items-center gap-3">
              {action.newsPapers && (
                <Newspaper className="w-5 h-5 text-muted-foreground group-hover:text-foreground transition-colors" />
              )}
              ...
              {action.vindicatedActions && (
                <BookOpenCheck className="w-4 h-4 text-muted-foreground group-hover:text-foreground transition-colors" />
              )}
              {action.videos && (
                <MonitorPlay className="w-4 h-4 text-muted-foreground group-hover:text-foreground transition-colors" />
              )}
            </div>
