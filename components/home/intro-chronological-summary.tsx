    ...
    <div className="flex flex-col h-full bg-background text-foreground">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 place-items-center">
        {chronological.map((year, index) => (
          <Link href={`/cronologico/${year}`} key={index} className="group">
      ...
      <div className="mt-8 pt-6 border-t border-border">
        <div className="flex items-center gap-3 mb-4">
          ...
          <span className="text-[10px] font-bold uppercase tracking-widest leading-none">
            {chronological.length} Años Catalogados
          </span>
        </div>
      </div>
    </div>
...
function CalendarSVG() {
  return (
    <svg
      ...
      className="drop-shadow-sm opacity-80 group-hover:opacity-100 transition-opacity"
    >
      <path
