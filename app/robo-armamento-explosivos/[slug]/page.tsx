...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      type: "article",
      locale: "es_UY",
      url: `https://www.pasadoreciente.com/robo-armamento-explosivos/${slug}`,
    ...
    alternates: {
      canonical: `https://www.pasadoreciente.com/robo-armamento-explosivos/${slug}`,
    },
  };
}
  ...
  return (
    <>
      <JsonLdArticle
        ...
        description={`${action.type} ocurrido el ${formattedDate}. Museo de la Memoria del Pasado Reciente - Hemeroteca digital con noticias escaneadas de prensa de los años 60 y 70.`}
        date={action.date}
        url={`https://www.pasadoreciente.com/robo-armamento-explosivos/${params.slug}`}
        type={action.type}
      />
        ...
        <header className="mb-12 border-b-4 border-foreground pb-6">
          <div className="flex items-center gap-2 mb-4">
            <span className="bg-primary text-white text-xs  lg:text-[12px] px-2 lg:px-4 py-0.5 lg:py-1 font-black uppercase tracking-widest">
              ROBO ARMAMENTO - EXPLOSIVOS / Uruguay
            </span>
            {/* <span className="text-xs font-bold text-muted-foreground uppercase tracking-widest">
            ROBO ARMAMENTO / Uruguay
          </span> */}
          </div>
          <h1
